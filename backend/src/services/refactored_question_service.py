...
class RefactoredQuestionService(ServiceBase):
    """
    Refactored service for managing trivia questions
    ...
    def __init__(
        ...
        Args:
            question_repository: Repository for data access
        ...
        """
        super().__init__(question_repository, category_repository)
    ...
    async def generate_and_save_questions(
        ...
        category_name = await self.category_utils.resolve_category_name(category)
        ...
        if '-' in category:  # Looks like UUID
            ...
            category_id = await self.category_utils.get_or_create_category_id(category)
    ...
    async def create_multi_difficulty_question_set(
        ...
        # Pre-generate shared guidelines to improve efficiency
        ...
        if categories and any(cat for cat in categories if '-' not in cat):
            ...
            for cat in categories:
                if '-' not in cat:  # Not a UUID, needs resolution
                    cat_id = await self.category_utils.get_or_create_category_id(cat)
                    if cat_id:
