...
#!/usr/bin/env python
# Example (topic provided): python3 test_incorrect_answers.py --pack-name "Science Facts" --topic "Physics" -n 3 -v
# Example (topic generated): python3 test_incorrect_answers.py --pack-name "Random Facts" -n 3 -v
...
import os
import traceback
import random # Added for topic selection
...
# --- Helper Functions (print_step, print_json, make_request) remain the same ---
...
def make_request(method: str, endpoint: str, json_data: Optional[Dict] = None, params: Optional[Dict] = None) -> Optional[requests.Response]:
    ...
    try:
        response = requests.request(method, url, json=json_data, params=params, timeout=120) # Increased timeout for LLM calls
...
def create_or_get_pack(pack_name: str) -> Optional[str]:
    ...
    # Check if pack exists by listing and matching name (case-insensitive)
    ...
    data = {
        "name": pack_name,
        "description": f"Test pack for incorrect answer generation",
        "price": 0.0,
        "creator_type": "system"
    }
...
def generate_and_select_topic(pack_id: str) -> Optional[str]:
    ...
    data = {
        "num_topics": 1 # Generate just one topic for simplicity in the test
    }
...
def add_topic_to_pack(pack_id: str, topic_name: str) -> bool:
    ...
    # Get existing topics first to decide which endpoint to use
    ...
    if topic_name in existing_topics:
         print(f"Topic '{topic_name}' already exists in the pack. Skipping add.")
         return True # Consider it success as the topic is present

    # Decide endpoint based on whether topics already exist
    endpoint = "/topics/"
    data = {"predefined_topic": topic_name}
    if existing_topics:
         print("Adding new topic to existing list using /additional endpoint.")
         endpoint = "/topics/additional" # Use the endpoint to add

    add_response = make_request("POST", f"/packs/{pack_id}{endpoint}", json_data=data)

    if add_response and add_response.status_code == 200:
        # Verify the topic is now in the list
        final_topics = add_response.json().get("topics", [])
        if topic_name in final_topics:
             print(f"{Colors.GREEN}Successfully added/confirmed topic '{topic_name}'{Colors.ENDC}")
             return True
        else:
             print(f"{Colors.FAIL}Topic '{topic_name}' was not found in the final list after add attempt.{Colors.ENDC}")
             return False
    else:
        print(f"{Colors.FAIL}Failed to add topic '{topic_name}'{Colors.ENDC}")
        return False


# --- Functions generate_pack_questions, generate_incorrect_answers remain the same ---
...
def generate_incorrect_answers(pack_id: str, num_questions_generated: int) -> bool:
    ...
    if response and response.status_code == 200:
        ...
        # Basic validation
        if processed_count >= num_questions_generated:
            print(f"{Colors.GREEN}Processed count matches or exceeds generated questions count.{Colors.ENDC}")
            return True
        else:
             print(f"{Colors.WARNING}Processed count ({processed_count}) is less than generated questions ({num_questions_generated}). Check logs.{Colors.ENDC}")
             return False # Consider this a potential issue
    else:
...
def run_test_flow(args: argparse.Namespace):
    ...
    if not pack_id:
        return # Stop if pack couldn't be obtained/created

    # --- MODIFIED TOPIC HANDLING ---
    ...
    if not topic_to_use:
        ...
        if not topic_to_use:
            print(f"{Colors.FAIL}Stopping test because topic generation failed.{Colors.ENDC}")
            return
    # --- END MODIFIED TOPIC HANDLING ---

    # Add the selected topic to the pack (this now also ensures it exists)
    if not add_topic_to_pack(pack_id, topic_to_use):
        print(f"{Colors.FAIL}Stopping test because topic could not be added/confirmed.{Colors.ENDC}")
        return

    # Ensure difficulty descriptions exist
    ...
    if not diff_resp or diff_resp.status_code != 200:
         print(f"{Colors.WARNING}Could not ensure difficulty descriptions exist. Question generation might use defaults.{Colors.ENDC}")

    # Generate questions using the selected topic
    generated_question_ids = generate_pack_questions(pack_id, topic_to_use, args.num_questions)
    if not generated_question_ids:
        print(f"{Colors.FAIL}Stopping test because question generation failed.{Colors.ENDC}")
        return

    # Now, test the incorrect answer generation
    success = generate_incorrect_answers(pack_id, len(generated_question_ids))

    print("\n" + "="*30 + " Test Flow Summary " + "="*30)
    if success:
        print(f"{Colors.GREEN}Incorrect Answer Generation Test Flow Completed Successfully for pack '{args.pack_name}' (ID: {pack_id}) using topic '{topic_to_use}'.{Colors.ENDC}")
    else:
        print(f"{Colors.FAIL}Incorrect Answer Generation Test Flow Failed for pack '{args.pack_name}' (ID: {pack_id}) using topic '{topic_to_use}'.{Colors.ENDC}")
    print("="*80)
...
def main(args: argparse.Namespace):
    ...
    if args.topic:
        print(f"Provided Topic: {args.topic}")
    else:
        print("Topic: Will be generated")
...
if __name__ == "__main__":
    ...
    # --- MODIFIED: Topic is now optional ---
